...
import { AdminRole, AdminStatus, Prisma, ShowtimeStatus } from '@prisma/client';

import { asyncHandler } from '../../utils/async-handler.js';
import {
  auditoriumCreateSchema,
  auditoriumLayoutUpdateSchema,
  auditoriumUpdateSchema,
  campaignPayloadSchema,
  inviteAdminSchema,
  platformSettingsPayloadSchema,
  seatLayoutSchema,
  showtimePayloadSchema,
...
import {
  ...
  mapAuditorium,
  mapBooking,
  mapNotificationCampaign,
  mapNotificationSegment,
  mapPlatformSettings,
  mapSettlement,
  mapShowtime,
  showtimeStatusFromClient,
  mapSeatLayoutJson,
  calculateSeatMetrics,
} from '../../modules/admin/admin.mappers.js';
...
const decimal = (value: Prisma.Decimal | number) =>
  typeof value === 'number' ? new Prisma.Decimal(value) : new Prisma.Decimal(value.toNumber());
  ...
  asyncHandler(async (req, res) => {
    ...
    const showtime = await prisma.showtime.create({
      data: {
        ...
        status: showtimeStatusFromClient(payload.status) as ShowtimeStatus,
  ...
  asyncHandler(async (req, res) => {
    ...
    const updated = await prisma.showtime.update({
      ...
      data: {
        ...
        status: payload.status ? (showtimeStatusFromClient(payload.status) as ShowtimeStatus) : showtime.status,
...
const addBookingAudit = async (bookingId: string, type: string, message: string) => {
    ...
    if (existingUser) {
      adminUser = await prisma.adminUser.upsert({
        ...
        update: {
          ...
          roles: {
            deleteMany: {},
            create: roles.map((role) => ({ role: role as AdminRole })),
          },
        },
        create: {
...
operationsRouter.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (payload.status) {
      updateData.status = payload.status.toUpperCase() as AdminStatus;
    }
    if (payload.name) {
    ...
    if (payload.roles) {
      updateData.roles = {
        deleteMany: {},
        create: payload.roles.map((role) => ({ role: role.toUpperCase() as AdminRole })),
      };
    }
