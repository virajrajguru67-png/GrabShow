...
import { bookingReferenceSchema, createBookingSchema, movieIdentifierSchema, movieListQuerySchema, showtimeParamSchema, } from '../modules/catalog/catalog.schemas.js';
import { buildSeatMapWithAvailability, buildSeatTypeMap, toMovieDetail, toMovieSummary, } from '../modules/catalog/catalog.mappers.js';
import { mapBooking } from '../modules/admin/admin.mappers.js';
import { badRequest, notFound } from '../errors/app-error.js';
export const catalogRouter = Router();
const DEFAULT_LIMIT = 20;
const resolveMovieIdentifier = async (identifier) => {
...
catalogRouter.get('/movies', asyncHandler(async (req, res) => {
...
catalogRouter.get('/movies/:identifier', asyncHandler(async (req, res) => {
...
catalogRouter.get('/showtimes/:id/seats', asyncHandler(async (req, res) => {
    ...
    const layout = (showtime.auditorium.layoutJson ?? { rows: [] });
...
catalogRouter.post('/bookings', asyncHandler(async (req, res) => {
    ...
    const booking = await prisma.$transaction(async (tx) => {
        ...
        for (const tier of showtime.pricingTiers) {
            ...
            const seatTypes = Array.isArray(tier.seatTypes) ? tier.seatTypes : [];
...
catalogRouter.get('/bookings/:reference', asyncHandler(async (req, res) => {
